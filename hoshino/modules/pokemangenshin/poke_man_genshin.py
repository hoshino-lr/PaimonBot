...
POKE_TIP_LIMIT = 10  # 到达每日掉落上限后的短时最多提示次数
...
for image in image_list:
    ...
    if chara_id not in _Genshin_Impact_data.CHARA_NAME:
        continue
    star = image[3]
...
async def send_card(bot, ev: CQEvent):
    ...
    if uid == 1786944781:
        pass
        # db.add_card_num(665601009, 784108864, 133, 1)
        # db.add_card_num(665601009, 784108864, 135, 1)
        # db.add_card_num(665601009, 1178805308, 107, 1)
        await bot.send(ev, f'补偿成功')
    else:
        await bot.send(ev, f'您没有权限进行补偿')


@sv.on_fullmatch("取消补偿")
async def send_card(bot, ev: CQEvent):
    uid = ev.user_id
    if uid == 1786944781:
        pass
        # db.delete_card_num(665601009, 784108864, 133, 1)
        # db.delete_card_num(665601009, 784108864, 135, 1)
        # db.delete_card_num(665601009, 1178805308, 127, 1)
        # await bot.send(ev, f'补偿')
    else:
        await bot.send(ev, f'您没有权限进行补偿')
