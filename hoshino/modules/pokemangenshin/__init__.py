...
class CardRecordDAO:
    ...
    def _create_table(self):
        with self.connect() as conn:
            ...
            conn.execute(
                "CREATE TABLE IF NOT EXISTS gacha_10"
                "(gid INT NOT NULL, uid INT NOT NULL, num INT NOT NULL, PRIMARY KEY(gid, uid))"
            )
    ...
    def get_gacha_num(self, gid, uid):
        with self.connect() as conn:
            r = conn.execute(
                "SELECT num FROM gacha_10 WHERE gid=? AND uid=?", (gid, uid)
            ).fetchone()
            return r if r else [0]

    def add_gacha_num(self, gid, uid, increment=1):
        num = self.get_gacha_num(gid, uid)
        num = num[0] + int(increment)
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO gacha_10 (gid, uid,num) VALUES (?, ?, ?)",
                (gid, uid, num),
            )
        return num

    def add_delete_num(self, gid, uid):
        num = self.get_gacha_num(gid, uid)
        num = 0
        with self.connect() as conn:
            conn.execute(
    ...
    def delete_card_num(self, gid, uid, cid, increment=1):
        num = self.get_card_num(gid, uid, cid)
        num -= increment
        with self.connect() as conn:
            conn.execute(
    ...
    def change_card_num(self, uid, gid, cid):
        num = 7
        with self.connect() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO card_record (gid, uid, cid, num) VALUES (?, ?, ?, ?)",
...
class ExchangeRequestMaster:
    ...
    def get_last_exchange_request_time(self, gid, uid):
        return self.last_exchange_request[(gid, uid)].request_time if (gid,
                                                                       uid) in self.last_exchange_request else datetime(
            2020, 4, 17, 0, 0, 0, 0)
